...
The customer screens now use the corrected accessible blue-accent neumorphic KK10P foundation: centralized pale/blue tokens, canvas-matched ordinary controls, neutral paired shadows, borderless tactile native buttons with focus-only rings, always-concave Login support actions, true inset pressed states, raised field bodies and icon tiles. Shared feedback settles in 70 ms without a global debounce; native gesture cancellation and controller single-flight guards protect real actions. Raised cards separate their shadow backing from the opaque pale face to avoid physical-device color wash. The canonical [KK10P mobile design direction](banking-lab/docs/05_Design/kk10p-mobile-ui-design-system.md) and [implemented UI foundation plan](banking-lab/docs/02_Planning/plan-kk10p-mobile-ui-foundation.md) describe the current boundary. Analysis, 33 focused checks and all 126 low-memory Flutter tests pass; normal rest/press/rapid-tap and 200% top/scrolled states passed ADB review with font restoration confirmed, while Chris's visual acceptance and Home/account TalkBack listening remain pending. Original flowcharts and Word files in `docs/00_Drafts/` are reference ideas, not current behavior contracts.
